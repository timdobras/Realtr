<script lang="ts">
  ...
  let { property, thumbnails = [], totalImageCount = 0, onUpdate, onDelete }: Props = $props();
      ...
      {:else}
        <div class="grid h-full grid-cols-3 grid-rows-2 gap-0.5">
          {#each thumbnails.slice(0, 6) as thumbnail, i}
            <div class="bg-background-200 relative overflow-hidden">
              <img src={thumbnail} alt="Thumbnail {i + 1}" class="h-full w-full object-cover" />
              {#if i === 5 && totalImageCount > 6}
                <div class="absolute inset-0 flex items-center justify-center bg-black/60">
                  <span class="text-lg font-semibold text-white">+{totalImageCount - 6}</span>
                </div>
              {/if}
            </div>
          {/each}
