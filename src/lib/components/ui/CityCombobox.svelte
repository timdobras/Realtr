...
>
  <div class="relative">
    ...
    />
    <Combobox.Trigger
      class="text-foreground-500 hover:text-foreground-700 absolute end-2 top-1/2 -translate-y-1/2"
    >
  ...
  <Combobox.Portal>
    ...
    >
      <Combobox.Viewport class="p-1">
        ...
        {:else}
          {#if !isNewCity}
            <span class="text-foreground-500 block px-2 py-1.5 text-center text-xs"
              >No cities found</span
            >
          {/if}
        {/each}
