	...
	<DialogContent class="max-w-lg overflow-hidden rounded-xl">
		...
		<div class="border-background-200 border-b px-6 py-4">
			<div class="flex items-center gap-3">
				...
				<div>
					...
					<p class="text-foreground-500 text-sm">Auto-Straighten feature needs additional components</p>
				</div>
			</div>
		</div>
		...
		<div class="px-6 py-5">
			...
			{:else if isRunning}
				<div class="flex flex-col items-center py-6">
					...
					<p class="text-foreground-400 mt-2 text-xs">This may take a few minutes. A Windows admin prompt will appear.</p>
				</div>
			{:else}
				<div class="space-y-4">
					<p class="text-foreground-700 text-sm">
						The <strong>Auto-Straighten Images</strong> feature requires OpenCV to be installed on your system. This is a one-time setup that will:
					</p>

					<ul class="text-foreground-600 space-y-2 text-sm">
						<li class="flex items-start gap-2">
							<svg class="text-accent-500 mt-0.5 h-4 w-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<div class="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
						<div class="flex items-start gap-2">
							...
							<p class="text-xs text-amber-800">
								<strong>Administrator access required.</strong> Windows will ask for permission to install the components.
							</p>
						</div>
					</div>
		...
		{#if !setupComplete && !isRunning}
			<div class="border-background-200 flex items-center justify-between border-t px-6 py-4">
				<button onclick={handleSkip} class="text-foreground-500 hover:text-foreground-700 text-sm transition-colors">
					Skip for now
				</button>
				<button onclick={runSetup} class="bg-accent-500 hover:bg-accent-600 rounded-lg px-5 py-2 text-sm font-medium text-white transition-colors">
					Install Components
				</button>
			</div>
		{/if}
