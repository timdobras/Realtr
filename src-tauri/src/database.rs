...
// `pub(super)` so submodules of `database` can call it without re-exporting.
pub(super) fn get_database_pool(app: &tauri::AppHandle) -> Result<&SqlitePool, String> {
...
pub(super) fn get_base_path_for_status(
...
pub(super) fn folder_path_to_pathbuf(folder_path: &str) -> PathBuf {
...
pub(super) async fn get_property_base_path(
    app: &tauri::AppHandle,
    folder_path: &str,
    status: &str,
) -> Result<PathBuf, String> {
...
// Watermark commands and clear_folder_images helper moved to database/watermark.rs
...
// Set commands and add_directory_to_zip helper moved to database/sets.rs
